...
void HttpClient::constructRangeHeader(size_t start, size_t end) {
    ...
    if (end > 0) {
        MPHTTP_ASSERT(start <= end,
                      "HttpClient : construct header fail, start > end");
    ...
    output_buffer.append(request_header.method + " " + request_header.url +
                         " HTTP/1.1\r\n");
...
bool HttpClient::processResponseLine(const char *begin, const char *end) {
    ...
    MPHTTP_ASSERT(index != std::string::npos,
                  "Httpclient : fail to parse http version.");
    version = response_line.substr(0, index);

    response_line = response_line.substr(index + 1);
    index = response_line.find_first_of(" ");
    MPHTTP_ASSERT(index != std::string::npos,
                  "Httpclient : fail to parse http status code.");
    ...
    if (task_buffer_->task_type_ != MpHttpType::kHasRangeHeader &&
        ...
        if (task_buffer_->task_type_ == MpHttpType::kNotUse) {
            std::string content_length_header =
                response_.getHeader("Content-Length");
            if (content_length_header != "") {
                size_t length = 0;
                if (sscanf(content_length_header.c_str(), "%zu", &length) < 1) {
                    MPHTTP_FATAL(
                        "HttpClient GetSize : fail to get "
            ...
            if (result != "") {
                ...
                if (sscanf(result.c_str(), "bytes %zu-%zu/%zu", &start, &end,
                           &file_size) < 3) {
                    MPHTTP_FATAL(
                        "HttpClient GetSize : fail to get "
...
void HttpClient::onBody(timestamp_t recv_time) {
    ...
    size_t time_eclapse = std::chrono::duration_cast<std::chrono::milliseconds>(
                              recv_time - task_buffer_->start_time_)
                              .count();
    // MPHTTP_LOG(debug, "HttpClient : time eclapse = %zd, end = %zd",
    //            time_eclapse, end - task_buffer_->start_);

    size_t length = end - range.start - range.received;

    task_buffer_->memcpyFrom(input_buffer.peek(), range.start + range.received,
                             end);
...
void HttpClient::onReadable(timestamp_t received_time, bool is_end_of_stream) {
    if (read_state == kInitial) {
        mp_->UpdateRTT(send_time_, received_time);
        read_state = kReadingResponseLine;
    }
    if (read_state == kReadingResponseLine || read_state == kReadingHeaders) {
...
void HttpClient::CloseCallback() {
    loop_->enqueueCallback([this]() {
        // refcnt decreases to zero, sock_fd would be unregistered from the
