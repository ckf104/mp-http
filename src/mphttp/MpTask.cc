...
void MpTask::setMpHttpType(const Request& request) {
  if (request.method.compare("GET") != 0) {
    task_type_ = MpHttpType::kNotUse;
  } else if (request.headers.count("Range") == 1) {
    int ret = sscanf(request.headers.at("Range").c_str(), 
                     "bytes=%zd-%zd", &start_, &end_);

    if (ret < 2) {
      task_type_ = MpHttpType::kNotUse;
    } else if (end_ - start_ < threshold) {
    ...
    if (index != string::npos 
        && request.url.substr(index).compare(".m4s") == 0) {
...
ssize_t MpTask::writeToSocket(int fd) {
  ...
  std::string response_line = response_.version_ + " "  + status_code + " " + status_message + "\r\n";

  if (detail::writen(fd, (void *)response_line.c_str(), response_line.size()) < 0) {
  ...
  for (const auto &header : response_.headers_) {
    ...
    if (header.first == "Content-Range") {
      ...
      if (task_type_ == MpHttpType::kHasRangeHeader) {
        result = "Content-Length: " + std::to_string(end_ - start_ + 1) + "\r\n"; 
      } else {
        result = header.first + ": " + header.second + "\r\n";
      }
    } else {
