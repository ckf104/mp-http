...
std::optional<Response> send_normal_request(StreamPtr &client,
                                            StreamPtr (&servers)[path_num],
                                            const Request &client_req,
                                            Body &req_body, Body &reply_body);
std::optional<Response> send_mp_request();
std::optional<Response>
send_mp_request(StreamPtr &client, StreamPtr (&servers)[path_num],
                const Request &client_req, Body &req_body, Body &reply_body,
...
public:
  ...
  static bool sendReqeust(Rio_t &w, const Request &r, const Body &body) {
    ...
    if (w.rio_writen((void *)buf, strlen(buf)) < 0 ||
        w.rio_writen(body.content.get(), body.length) < 0)
      return false;
    return true;
  }
...
void doit(int client_fd) {
  ...
  StreamPtr serverStream[path_num] = {HttpStream::generateStream(0),
                                      HttpStream::generateStream(0)};
  if (!clientStream) {
  ...
  while (1) {
    ...
    if (!req_ptr) { // client close the connection
    ...
    if ((mp_http_type = need_mp_http(req, byte_range)) == mp_http::not_use) {
      response = send_normal_request(clientStream, serverStream, req, req_body,
                                     reply_body);
      if (!response) {
      ...
      response = send_mp_request(clientStream, serverStream, req, req_body,
                                 reply_body, mp_http_type, byte_range);
      if (!response) {
  ...
  std::optional<Response> send_normal_request(
      StreamPtr & client, StreamPtr(&servers)[path_num],
      const Request &client_req, Body &req_body, Body &reply_body) {
    ...
    HttpStream *server =
        *servers[0].get() ? servers[0].get() : servers[1].get();
  ...
  std::optional<Response> send_mp_request(
      StreamPtr & client, StreamPtr(&servers)[path_num],
      const Request &client_req, Body &req_body, Body &reply_body, mp_http type,
      const std::pair<uint64_t, uint64_t> &byte_range) {
    return send_normal_request(client, servers, client_req, req_body,
                               reply_body);
  }
  ...
  mp_http need_mp_http(const Request &req,
                       std::pair<uint64_t, uint64_t> &range) {
    if (req.method.compare("GET") != 0)
      return mp_http::not_use;
    if (req.headers.count(range_header) == 1) {
      int ret = sscanf(req.headers.at(range_header).c_str(), "bytes=%ld-%ld",
